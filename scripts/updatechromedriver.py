...
def get_chromedriver(version, platform):
    ...
    downloads = [
        r["downloads"]["chromedriver"]
        for r in releases["versions"]
    ...
    if not downloads:
        ...
        print(
            f"CRITICAL chromedriver: no downloads found for Chrome {version} on {platform}"
        )
        print(
            f"CRITICAL Details see https://googlechromelabs.github.io/chrome-for-testing/"
        )
        print(
            f"CRITICAL The last known good version is {good_version} (channel Stable)"
        )
        exit(1)
    ...
    if not match:
        raise ValueError(
            "no chromedriver found for version {} and platform {}".format(
                version, platform
            )
        )
    cmds = [
