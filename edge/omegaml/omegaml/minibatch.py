...
def time_emiter():
    ...
    while True:
        ...
        last_read = stream.last_read
        now = datetime.datetime.now()
        max_read =  last_read + datetime.timedelta(seconds=stream.interval)
        if now > max_read:
            qs = Buffer.objects.no_cache().filter(created__gt=last_read, created__lte=max_read)
            data = []
            for b in qs:
                data.append(b.data)
                b.modify(processed=True)
            if data:
                window = Window(stream=stream.name, data=data).save()
                #process(window)
            stream.modify(last_read=max_read)
        time.sleep(stream.interval / 2.0)
...
def count_emiter():
    ...
    while True:
        stream = Stream.get_or_create('test')
        last_read = stream.last_read
        qs = Buffer.objects.no_cache().filter(processed=False).limit(stream.interval)
        data = []
        for b in qs:
            data.append(b.data)
            b.modify(processed=True)
        if len(data) >= stream.interval:
            window = Window(stream=stream.name, data=data).save()
            #process(window)
        stream.modify(last_read=datetime.datetime.now())
        time.sleep(1)
