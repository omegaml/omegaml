...
if [ -f requirements.txt ]; then
    ...
    pip install -r requirements.txt  --ignore-installed --exists-action=w --allow-all-external | indent
fi

if [ -f pip-requirements.extra ]; then
    puts-step "Installing extra requirements using Pip"
    pip install -r pip-requirements.extra --ignore-installed --exists-action=w --allow-all-external | indent
fi