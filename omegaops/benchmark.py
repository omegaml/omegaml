...
def insert_batch(db, collection, object):
    for i in range(per_trial // batch_size):
        db[collection].insert([dict(o) for o in [object] * batch_size])
...
def timed(name, function, args=[], setup=None):
    ...
    best_time = min(times)
    print("{0:s}{1:f}".format(name + (60 - len(name)) * ".", per_trial / best_time))
    return best_time


def main():
    url = sys.argv[1] if len(sys.argv) > 1 else 'localhost'
    c = mongo_client.MongoClient(url, connectTimeoutMS=60*1000)  # jack up timeout
