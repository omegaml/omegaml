        ...
        assistants = [
            {
                'name': prompt.name,
                'title': (
                    prompt.attributes.get('title')
                    or prompt.attributes.get('docs', '').split('\n')[0].replace('#', '').strip()
                ),
                'attributes': prompt.attributes,
            }
            for prompt in prompts
            if (
                not prompt.name.startswith('_')
                and name in prompt.attributes.get('permissions', {}).get('groups', ['sibyl'])
            )
        ]
        return render_template('dashboard/genai/chatapp.html', name=name, assistants=assistants, segment=self.segment)
