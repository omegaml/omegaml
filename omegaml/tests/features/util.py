...
istrue = lambda v: (
    (v.lower() in ("yes", "1", "y", "true", "t")) if isinstance(v, str) else bool(v)
)
...
class Notebook:
    ...
    def create_folder(self, name=""):
        ...
        self.file_browser
        br.find_by_xpath(
            "//button[@data-command='filebrowser:create-new-directory']"
        ).first.click()
        sleep(2)
    ...
    def open_notebook(self, name, retry=5):
        ...
        item = [
            el
            for el in br.find_by_css(".jp-DirListing-itemText")
            if el.value.startswith(name)
        ][0]
        item.double_click()
    ...
    def new_cell_exec(self, code):
        self.nbcell.type(
            ADD_CELL_BELOW + ACTIVATE_CELL + self._clean_code(code) + EXEC_CELL
        )
