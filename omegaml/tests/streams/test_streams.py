...
class StreamsTestCase(OmegaTestMixin, TestCase):
    ...
    def test_streams_autoattach_runtime(self):
        ...
        stream = om.streams.get(
            'test', streaming=dict(window=5), source=(om, 'runtime', dict(events=['*'])), autoattach=False
        )
        stream.stop()
        meta = om.streams.metadata('test')
        self.assertEqual(
            meta.kind_meta,
            {
                ...
                'attach': {'source': 'runtime', 'source_kwargs': {'events': ['*']}},
            },
        )
    ...
    def test_streams_autoattach_dataset(self):
        ...
        stream = om.streams.get('test', streaming=dict(window=5), source=(om, 'foobar'), autoattach=False)
        meta = om.streams.metadata('test')
        self.assertEqual(
            meta.kind_meta,
            {
                ...
                'attach': {'source': 'foobar', 'source_kwargs': {}},
            },
        )
    ...
    def test_streams_autoclean(self):
        ...
        # specify streaming and source kwargs
        om.datasets.put([1, 2, 3], 'foobar')
        stream = om.streams.get('test', streaming=dict(window=5), source=(om, 'foobar'), autoattach=False, max_age=0.1)
        meta = om.streams.metadata('test')
        stream.stop()
        self.assertEqual(
            meta.kind_meta,
