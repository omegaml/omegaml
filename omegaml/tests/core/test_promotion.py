...
class PromotionMixinTests(OmegaTestMixin, TestCase):
    ...
    def test_promotion_to_other_db_works(self):
        ...
        [
            other.models.drop(name, force=True)
            for name in other.models.list(include_temp=True)
        ]
        [
            other.datasets.drop(name, force=True)
            for name in other.datasets.list(include_temp=True)
        ]
        reg = LinearRegression()
        reg.coef_ = 10
        # try models
        ...
        om.models.put(reg, "mymodel")
        self.assertNotEqual(
            om.models.get("mymodel").coef_, other.models.get("mymodel").coef_
        )
        # try datasets
        om.datasets.put(["foo"], "foo")
