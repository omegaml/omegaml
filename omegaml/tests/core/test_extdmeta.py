...
from omegaml.mixins.store.extdmeta import (
    SignatureMixin,
    ModelSignatureMixin,
    ScriptSignatureMixin,
)
...
class ExtendedMetadataMixinTests(OmegaTestMixin, unittest.TestCase):
    ...
    def test_link_script_signature(self):
        ...
        with self.assertRaises(RuntimeError) as ex:
            om.runtime.script("myhandler").run(
                {
                    "foo": [1],
                }
            ).get()
        self.assertIn(
            "ValidationError({'foo': ['Not a valid string.']})", str(ex.exception)
        )
    ...
    def test_swagger_generator(self):
        ...
        om.models.link_dataset(
            "mymodel", Xname="testX", data_store=om.datasets, actions=["predict", "fit"]
        )
    ...
    def test_swagger_link_specs(self):
        ...
        specs = {
            ...
            "info": {"title": "omega-ml service", "version": "1.0.0"},
            "swagger": "2.0",
            "definitions": {
                "EmptyX": {"type": "object", "properties": {}},
                    ...
                    "post": {
                        ...
                        "responses": {
                            "200": {
                                ...
                                "schema": {
                                    "type": "array",
                                    "items": {"$ref": "#/definitions/mymodel_Y"},
                                },
                            },
                            "404": {
                                "description": "no description",
                                "schema": {
                                    "type": "array",
                                    "items": {"$ref": "#/definitions/mymodel_404"},
                                },
                            },
                            "400": {
                                "description": "no description",
    ...
    def test_link_exception_single(self):
        ...
        meta = om.models.metadata("mymodel")
        self.assertTrue(om.models.validate("mymodel", error=Exception("failure", 404)))
        with self.assertRaises(ValidationError):
            om.models.validate(
                "mymodel", error=Exception(dict(xmessage="failure"), 404)
            )
        # -- also test we can pass exception as the result (i.e. not raised)
        with self.assertRaises(ValidationError):
            om.models.validate(
                "mymodel", result=Exception(dict(xmessage="failure"), 404)
            )
        self.assertTrue(om.models.validate("mymodel", result=Exception("failure", 404)))
        # check swagger spec includes errors
        specs = om.runtime.swagger(format="dict", as_service=True)
        self.assertIn("paths", specs)
        self.assertIn("/api/service/mymodel", specs["paths"])
        self.assertIn(
            "404", specs["paths"]["/api/service/mymodel"]["post"]["responses"]
        )
    ...
    def test_link_errors_single(self):
        ...
        meta = om.models.metadata("mymodel")
        self.assertTrue(
            om.models.validate("mymodel", error=Exception({"message": "failure"}, 404))
        )
        with self.assertRaises(ValidationError):
        ...
        with self.assertRaises(ValidationError):
            om.models.validate(
                "mymodel", result=Exception({"xmessage": "failure"}, 404)
            )
        # check swagger spec includes errors
        specs = om.runtime.swagger(format="dict", as_service=True)
        self.assertIn("paths", specs)
    ...
    def test_link_errors_multiple(self):
        ...
        errors = [([ErrorSchema], 404)]
        om.models.link_datatype("mymodel", errors=errors)
        meta = om.models.metadata("mymodel")
        self.assertTrue(
            om.models.validate(
                "mymodel", error=Exception([{"message": "failure"}], 404)
            )
        )
        # -- not a list
        with self.assertRaises(ValidationError):
        ...
        with self.assertRaises(ValidationError):
            om.models.validate(
                "mymodel", error=Exception([{"xmessage": "failure"}], 404)
            )
        # check swagger spec includes errors
        specs = om.runtime.swagger(format="dict", as_service=True)
        self.assertIn("paths", specs)
