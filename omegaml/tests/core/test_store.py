...
class StoreTests(unittest.TestCase):
    ...
    def test_put_dataframe_timestamp(self):
        ...
        self.assertEqual(
            _created.replace(second=0, microsecond=0),
    ...
    def test_put_dataframe_multiindex(self):
        ...
        idx_names = humanize_index(idxs)
        self.assertIn(
            "asc__id_asc__idx#0_first_asc__idx#1_second_asc__om#rowid", idx_names
        )
    ...
    def test_store_with_attributes(self):
        ...
        store.put(
            df, "foo", append=False, attributes={"foo": "bax", "foobar": "barbar"}
        )
        meta = store.metadata("foo")
    ...
    def test_store_named_series(self):
        ...
        series = pd.Series(
            range(10), name="foo", index=(c for c in ascii_lowercase[0:10])
        )
        store = OmegaStore()
        store.put(series, "fooseries", append=False)
        series2 = store.get("fooseries")
    ...
    def test_store_tz_datetime(self):
        ...
        df = pd.DataFrame(
            {
                "y": pd.date_range(
                    "2019-10-01", periods=5, tz="US/Eastern", normalize=True
                )
            }
        )
        store = OmegaStore()
    ...
    @skip(
        "date ranges across dst period start/end do not return the original DatetimeIndex values"
    )
    def test_store_tz_datetime_dst(self):
        ...
        df = pd.DataFrame(
            {
                "y": pd.date_range(
                    "2019-11-01", periods=5, tz="US/Eastern", normalize=True
                )
            }
        )
        store = OmegaStore()
    ...
    def test_bucket(self):
        ...
        self.assertNotEqual(
            foo_store.get("myfile").read(), bar_store.get("myfile").read()
        )
    ...
    def test_help_docs(self):
        foo_store = OmegaStore(bucket="foo")
        reg = LinearRegression()
        foo_store.put(reg, "regmodel", attributes={"docs": "this is some text"})
        # get backend for different signatures
