...
from omegaml.mixins.store.imexport import (
    ObjectImportExportMixin,
    OmegaExportArchive,
    OmegaExporter,
)
...
class ImportExportMixinTests(OmegaTestMixin, unittest.TestCase):
    ...
    def test_model_multiple_versioned_export(self):
        ...
        om_restore.models.promote(
            "mymodel@version1", om.models, asname="mymodel@version1"
        )
        om_restore.models.promote(
            "mymodel@version2", om.models, asname="mymodel@version2"
        )
    ...
    def test_runtime_exporter_export_import_compressed(self):
        ...
        df = pd.DataFrame({"x": range(10)})
        om.datasets.put(df, "mydf", append=False)
        arcfile = OmegaExporter(om).to_archive(
            "/tmp/test", ["data/mydf"], compress=True
        )
    ...
    def test_runtime_exporter_export_import_bare_tarfile(self):
        ...
        om.datasets.put(df, "mydf", append=False)
        arcfile = OmegaExporter(om).to_archive(
            "/tmp/test", ["data/mydf"], compress="tar"
        )
        with self.assertRaises(FileNotFoundError):
            OmegaExporter(om_restore).from_archive("/tmp/test")
        meta = om_restore.datasets.metadata("mydf")
    ...
    def test_runtime_exporter_export_promote(self):
        ...
        with patch.object(om_restore, "_get_bucket") as meth:
            ...
            OmegaExporter(om_restore).from_archive(
                "/tmp/test", pattern="data/.*|models/.*", promote_to=om_restore
            )
            self.assertIn("mydf", om_restore.datasets.list())
