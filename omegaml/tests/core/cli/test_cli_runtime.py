...
class CliRuntimeTests(CliTestScenarios, OmegaTestMixin, TestCase):
    ...
    def test_cli_deploy_steps(self):
        ...
        expected = self.pretend_log(
            "DRY: om runtime  export models/reg datasets/sample"
        )
        self.assertLogContains("info", expected)
        self.cli(f"runtime deploy import --steps {deployfile} --dry")
        expected = self.pretend_log(
            "DRY: om runtime  import models/reg datasets/sample"
        )
        self.assertLogContains("info", expected)
        # actually do it
    ...
    def test_cli_restart_app(self):
        ...
        with patch("omegaml.client.cli.runtime.requests") as requests, patch(
            "omegaml.client.cli.runtime.get_omega"
        ) as get_omega:
            get_omega.return_value = om
            self.cli("runtime restart app apps/test")
            requests.get.assert_called()
            self.assertEqual(
                requests.get.call_args_list[0][0],
                ("local/apps/api/stop/testuser/test",),
            )
            self.assertEqual(
                requests.get.call_args_list[1][0],
                ("local/apps/api/start/testuser/test",),
            )
        ...
        with patch("omegaml.client.cli.runtime.requests") as requests, patch(
            ...
            self.assertEqual(
                requests.get.call_args_list[0][0],
                ("http://myapphub.com/apps/api/stop/testuser/test",),
            )
            self.assertEqual(
                requests.get.call_args_list[1][0],
                ("http://myapphub.com/apps/api/start/testuser/test",),
            )
