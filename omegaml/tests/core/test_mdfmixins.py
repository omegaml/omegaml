...
class MDataFrameMixinTests(TestCase):
    ...
    def test_apply_custom_project_simple(self):
        ...
        def complexfn(ctx):
            ctx.project(x={"$multiply": ["$x", 5]}, b={"$divide": ["$x", 5]})
            return ctx
    ...
    def test_apply_dict(self):
        ...
        mdf = om.datasets.getl("sample")
        mdf = mdf.apply(lambda v: dict(a=v["x"].dt.dayofweek, b=v["x"].dt.year)).value
    ...
    def test_apply_groupby_agg(self):
        ...
        self.assertEqual(list(expected[("v", "sum")].values), list(mdf["v_sum"].values))
        self.assertEqual(
            list(expected[("v", "mean")].values), list(mdf["v_avg"].values)
        )
    ...
    def test_apply_cache(self):
        ...
        mdf = om.datasets.getl("sample")
        cache_key = (
            mdf.apply(lambda v: v.groupby("x").agg(v=["sum", "mean", "std"]))
            .reset_cache()
            .persist()
        )
        cursor = mdf.apply(
            lambda v: v.groupby("x").agg(v=["sum", "mean", "std"])
        )._get_cached_cursor()
        expected = df.groupby("x").agg(dict(v=["sum", "mean", "std"]))
        ...
        self.assertNotEqual(
            list(expected[("v", "sum")].values), list(mdf["v_sum"].values)
        )
        self.assertNotEqual(
            list(expected[("v", "mean")].values), list(mdf["v_avg"].values)
        )
        self.assertNotEqual(
            list(expected[("v", "std")].values), list(mdf["v_std"].values)
        )
