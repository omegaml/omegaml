...
class OmegaLoggingTests(OmegaTestMixin, unittest.TestCase):
    ...
    def tearDown(self):
        self.assertTrue(
            all(m.name is not None for m in self.om.datasets.list(raw=True))
        )
    ...
    def test_simple_logger(self):
        ...
        for level in OmegaSimpleLogger.levels:
            ...
            self.assertTrue(
                len(df) == expected, "level {} did not have a record".format(level)
            )
            if expected:
    ...
    def test_loghandler(self):
        ...
        handler = OmegaLoggingHandler.setup(
            logger=pylogger, level="DEBUG", exit_hook=True
        )
        ...
        for level in ["INFO", "ERROR", "WARNING", "DEBUG"]:
            ...
            expected = 1
            self.assertEqual(
                len(df), expected, "expected 1 message for level {}".format(level)
            )
            if expected:
                self.assertEqual(df.iloc[0].msg, "{} message".format(level).lower())
                # text contains the formatted msg
