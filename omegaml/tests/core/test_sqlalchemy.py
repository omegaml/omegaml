...
class SQLAlchemyBackendTests(OmegaTestMixin, TestCase):
    ...
    def test_put_connection_with_secrets(self):
        ...
        # via vault
        om.datasets.put(
            dict(userid=getuser(), user="foobar"), ".omega/vault", append=False
        )
        conn = om.datasets.get("testsqlite", raw=True)
        self.assertIsInstance(conn, Connection)
    ...
    def test_connection_cache(self):
        ...
        cnx = "sqlite:///{user}.db"
        om.datasets.put(cnx, "testsqlite", kind=SQLAlchemyBackend.KIND)
        conn = om.datasets.get(
            "testsqlite", raw=True, secrets=dict(user="user"), keep=True
        )
        conn_ = om.datasets.get(
            "testsqlite", raw=True, secrets=dict(user="user"), keep=True
        )
        ...
        self.assertTrue(
            len(sqlalchemy.SQLAlchemyBackend._SQLAlchemyBackend__CNX_CACHE) == 0
        )
    ...
    def test_put_connection_with_sql(self):
        ...
        om.datasets.put(
            cnx_str, "foobar", sql="select * from foobar", kind=SQLAlchemyBackend.KIND
        )
    ...
    def test_query_in_clause(self):
        ...
        df_db = om.datasets.get("foobar", sqlvars={"x": [1, 2, 3], "y": [5, 6, 7]})
    ...
    def test_trusted_sqlvars(self):
        ...
        for trusted in [
            ...
            True,
            None,
            sha256(str(sqlvars).encode("utf-8")).hexdigest(),
        ]:
