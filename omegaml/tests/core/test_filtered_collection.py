...
class FilteredCollectionTests(TestCase):
    ...
    def test_injection(self):
        ...
        with warnings.catch_warnings(record=True) as wrn:
            ...
            self.assertIn(
                "$where clauses are not permitted and replaced by -where for security reasons.",
                warnlog,
            )
    ...
    def test_trusted_filter(self):
        ...
        for trusted in [
            ...
            True,
            None,
            sha256(str(filter).encode("utf-8")).hexdigest(),
        ]:
            with warnings.catch_warnings(record=True) as wrn:
                warnings.simplefilter("always")
                ...
                warnlog = str(list(w.message for w in wrn))
                self.assertIn(
                    "Your MongoDB query contains operators ['$in'] which may be unsafe if not sanitized.",
                    warnlog,
                )
                self.assertEqual(result, 4)
        with warnings.catch_warnings(record=True) as wrn:
            warnings.simplefilter("always")
            fcoll = FilteredCollection(
                self.coll, query=filter, trusted=signature(filter)
            )
            result = fcoll.count_documents()
            warnlog = str(list(w.message for w in wrn))
            self.assertNotIn(
