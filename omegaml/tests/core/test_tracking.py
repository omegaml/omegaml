...
class TrackingTestCases(OmegaTestMixin, unittest.TestCase):
    ...
    def test_tracking_predictions_explicit_metadata(self):
        ...
        lr = LogisticRegression(solver="liblinear", multi_class="auto")
        lr.fit(X, Y)
        om.models.put(lr, "mymodel", attributes={"tracking": {"default": "expfoo"}})
        om.runtime.model("mymodel").score(X, Y)
        tracker = om.runtime.experiment("expfoo")
        exp = tracker.experiment
        data = exp.data()
    ...
    def test_summary(self):
        ...
        self.assertEqual(
            set(summary.loc["metric"].index), {"accuracy", "mse", "latency"}
        )
    ...
    def test_batched_data(self):
        ...
        for raw, result_type in zip((True, False), (list, pd.DataFrame)):
            ...
            total_rows = sum(
                len(rows) for rows in data if isinstance(rows, result_type)
            )
    ...
    def test_since_filter(self):
        ...
        # only last 5 hours
        data = exp.data(
            event="metric", key="acc", since=dt_start + datetime.timedelta(hours=5)
        )
        self.assertEqual(len(data), 5)
        # only last hour
        data = exp.data(
            event="metric", key="acc", since=dt_start + datetime.timedelta(hours=9)
        )
        self.assertEqual(len(data), 1)
        # all data since start - 1 hour
        data = exp.data(
            event="metric", key="acc", since=dt_start - datetime.timedelta(hours=1)
        )
        self.assertEqual(len(data), 10)
        ...
        data = exp.data(
            ...
            event="metric",
            key="acc",
            since=dt_start + datetime.timedelta(hours=5),
        )
        self.assertEqual(len(data), 5)
