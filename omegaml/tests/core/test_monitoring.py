...
class DriftMonitoringTests(OmegaTestMixin, TestCase):
    ...
    def test_model_drift_stats(self):
        ...
        self.assertEqual(
            set(summary["statistic"].values), {"ks", "mean", "wasserstein"}
        )
    ...
    def test_dataset_drift(self):
        ...
        self.assertEqual(
            set(df.columns),
            set(data["info"]["num_columns"] + data["info"]["cat_columns"]),
        )
    ...
    def _setup_model(
        self, exp_name="test", model_name="test", save_xy=False, autotrack=False
    ):
        om = self.om
        with om.runtime.experiment(exp_name, autotrack=autotrack) as exp:
            ...
            for i in range(100):
                ...
                if i == 0:
                    ...
                    y = (
                        np.dot(X, np.random.random(size=(4, 1)) * 1000)
                        + np.random.random() * 5
                    )
                    if save_xy:
                        om.datasets.put(X, f"X_{i}")
    ...
    def test_model_drift_xy_groupby(self):
        ...
        # Create the pipeline
        pipeline = Pipeline(
            [("preprocessor", preprocessor), ("logreg", LogisticRegression())]
        )
        with om.runtime.experiment("test") as exp:
            mon = DataDriftMonitor("foo", store=om.datasets, tracking=exp)
        mon = DataDriftMonitor(tracking=exp, store=om.datasets)
    ...
    @skip(
        "autotracking for virtualobj models is not yet implemented, pending virtualobj.model support"
    )
    ...
    def test_most_recent_snapshot(self):
        ...
        for i in range(5):
            ...
            self.assertEqual(
                snapshot1["info"], mon._most_recent_snapshots(n=2)[-2]["info"]
            )
            self.assertEqual(
                snapshot2["info"], mon._most_recent_snapshots(n=2)[-1]["info"]
            )
            self.assertEqual(
                snapshot1["info"]["dt"], mon._most_recent_snapshot_time(n=2)
            )
