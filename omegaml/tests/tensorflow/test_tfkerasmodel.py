...
class TensorflowKerasBackendTests(OmegaTestMixin, TestCase):
    def setUp(self):
        ...
        from omegaml.backends.tensorflow.tfkerassavedmodel import (
            TensorflowKerasSavedModelBackend,
        )

        self.om = Omega()
        self.om.models.register_backend(
            TensorflowKerasBackend.KIND, TensorflowKerasBackend
        )
        self.om.models.register_backend(
            TensorflowKerasSavedModelBackend.KIND, TensorflowKerasSavedModelBackend
        )
        self.clean()
    ...
    def test_save_load(self):
        ...
        self.assertTrue(
            np.all(np.equal(model_.get_weights()[0], (model.get_weights()[0])))
        )
    ...
    def test_fit_tpu(self):
        ...
        result = (
            om.runtime.model("keras-model").fit(x_test, y_test, tpu_specs=True).get()
        )
        self.assertTrue(result.startswith("<Metadata:"))
