...
class TensorflowSavedModelBackendTests(OmegaTestMixin, TestCase):
    def setUp(self):
        ...
        self.om = Omega()
        self.om.models.register_backend(
            TensorflowSavedModelBackend.KIND, TensorflowSavedModelBackend
        )
        self.clean()
    ...
    def _build_model(self):
        ...
        train_input_fn = _tffn("numpy_input_fn")(
            x={"X_input": x_train}, y=y_train, num_epochs=1, shuffle=False
        )
    ...
    @skipIf(
        module_available("tensorflow") and tf_in_eager_execution(),
        "cannot run in eager mode",
    )
    def test_save_load_tf_example(self):
        from omegaml.backends.tensorflow.tfsavedmodel import (
            TensorflowSavedModelPredictor,
        )
        ...
        def serving_input_receiver_fn():
            ...
            serialized_tf_example = tf.placeholder(
                dtype=tf.string, shape=[default_batch_size], name="X_input"
            )
        ...
        om.models.put(
            model, "estimator-savedmodel", serving_input_fn=serving_input_receiver_fn
        )
        ...
        for i in range(x_test.shape[0]):
            example = tf.train.Example(
                features=tf.train.Features(
                    feature={
                        "X_input": tf.train.Feature(
                            float_list=tf.train.FloatList(value=x_test[i, :])
                        )
                    }
                )
            )
    ...
    def test_save_load_native(self):
        ...
        def serving_input_receiver_fn():
            placeholder = tf.placeholder(
                dtype=np.float32, shape=(default_batch_size, 20), name="X_input"
            )
