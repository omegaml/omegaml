...
def make_data():
    ...
    train_data = pd.DataFrame(np.random.random_sample((20, 5)), columns=columns).astype(
        int
    )
    test_data = pd.DataFrame(np.random.random_sample((20, 5)), columns=columns).astype(
        int
    )
    # separate train data
...
def make_estimator_fn():
    ...
    def make_estimator(model_dir=None):
        ...
        classifier = tf.estimator.LinearClassifier(
            feature_columns=feature_columns, n_classes=3, model_dir=model_dir
        )
        return classifier
...
class TFEstimatorModelBackendTests(OmegaTestMixin, TestCase):
    def setUp(self):
        ...
        self.om.models.register_backend(
            TFEstimatorModelBackend.KIND, TFEstimatorModelBackend
        )
        self.clean()
    ...
    def test_save_load_fitted(self):
        ...
        self.assertTrue(
            np.allclose(predict_r[0]["probabilities"], predict[0]["probabilities"])
        )
    ...
    def test_save_load_fitted_estimator(self):
        ...
        predict = [v for v in estmdl.predict(test_x)]
        meta = om.models.put(
            TFEstimatorModel(estimator_fn=make_estimator_fn(), model=estmdl.estimator),
            "estimator-model",
        )
        # restore and use
        estmdl_r = om.models.get("estimator-model")
    ...
    def test_predict_from_objecthandler(self):
        ...
        om.models.put(estmdl, "estimator-model")
        meta = (
            om.runtime.model("estimator-model")
            .fit("train_data{Xname=train_x,Yname=train_y}")
            .get()
        )
        self.assertTrue(meta.startswith("<Metadata:"))
