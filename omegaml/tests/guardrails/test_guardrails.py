...
class GuardrailPolicyTests(OmegaTestMixin, TestCase):
    ...
    def test_state_flow(self):
        ...
        # ------------------------------------------------------------------
        SCANNERS = {
            "haiku_in_prompt": lambda messages, **kwargs: ("haiku" in messages[-1]["content"], "haiku not in messages"),
            "evil_in_prompt": lambda messages, **kwargs: ("evil" not in messages[-1]["content"], "evil in messages"),
            "bad_in_prompt": lambda messages, **kwargs: ("bad" not in messages[-1]["content"], "bad in messages"),
        }
        ...
        def guardrails(model=None):
            ...
            policy.add_eval(state="converse", kind="intent", fn=lambda messages, **kwargs: ("finalize", {}))
            policy.add_eval(state="converse", kind="state", fn=lambda messages, **kwargs: ("stop", {}))
        ...
        self.assertIn(
            ("all", "finalize", {}),
            policy_B.context.get("intents", []),
            "context.intents should contain the fired intent",
        )
    ...
    def test_save_guardrails(self):
        ...
        om = self.om
        om.models.put(guardrails, "policy/myrails")
        rails = om.models.get("policy/myrails")
        myrails = rails()
        self.assertIsInstance(myrails, GuardrailPolicy)

    def test_set_scanners_fn(self):
        def myscanner(policy, messages):
            return True

        class MyPolicy:
            def eval(self, messages, step=None, policy=None, model=None):
                return True
        ...
        class MyOtherPolicy:
            ...
            def __repr__(self):
                return 'myotherpolicy:1'

        policy = GuardrailPolicy('myrails')
        policy.scanners = [myscanner, MyPolicy(), MyOtherPolicy()]
        self.assertIn('myscanner', policy.scanners)
        self.assertIn('mypolicy', policy.scanners)
        self.assertIn('myotherpolicy:1', policy.scanners)
