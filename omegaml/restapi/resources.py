...
from omegaml.backends.restapi.asyncrest import (
    AsyncTaskResourceMixin,
    AsyncResponseMixin,
    resolve,
)
...
TaskOutput = strict(api).model(
    "TaskOutput",
    {"task_id": fields.String, "status": fields.String, "response": fields.Raw},
)
...
DatasetQueryOutput = api.model(
    "DatasetQueryOutput", {"data": fields.Raw, "index": fields.Nested(DatasetIndex)}
)
...
class TaskResource(OmegaResourceMixin, AsyncTaskResourceMixin, Resource):
    ...
    def resolve_resource_method(self, taskid, context):
        ...
        URI_METHOD_MAP = {
            r"/api/.*/model/.*/.*/?$": (
                "_generic_model_resource",
                "prepare_result",
                {"pk": "model_id"},
            ),
            r"/api/.*/job/.*/run/?$": (
                "_generic_job_resource",
                "prepare_result",
                {"pk": "job_id"},
            ),
            r"/api/.*/script/.*/run/?$": (
                "_generic_script_resource",
                "prepare_result",
                {"pk": "script_id"},
            ),
            # services are also exposed as top-level services, see ServiceResoure
            r"/api/.*/service/.*/(.*)?$": (
                "_generic_service_resource",
                "prepare_result",
                {"pk": "resource_id"},
            ),
            r"/api/service/.*/(.*)?$": (
...
@api.route(
    "/api/v1/model/<path:model_id>/", defaults={"action": "metadata"}, methods=["GET"]
)
class ModelResource(OmegaResourceMixin, AsyncResponseMixin, Resource):
    ...
    def get(self, model_id, action=None):
        return self.create_response_from_resource(
            "_generic_model_resource", action, "model", model_id
        )
...
class ScriptResource(OmegaResourceMixin, AsyncResponseMixin, Resource):
    ...
    def post(self, script_id):
        return self.create_response_from_resource(
            "_generic_script_resource", "run", "script", script_id
        )


@api.route(
    "/api/service/<path:resource_id>",
    defaults={"action": "*"},
...
@api.route(
    "/api/service/<path:resource_id>/<string:action>",
    methods=["GET", "PUT", "POST", "DELETE"],
)
@api.route(
    "/api/v1/service/<path:resource_id>",
    defaults={"action": "*"},
    methods=["GET", "PUT", "POST", "DELETE"],
)
@api.route(
...
@api.route(
    "/api/v1/service/<path:resource_id>/<string:action>",
    methods=["GET", "PUT", "POST", "DELETE"],
)
...
class ServiceResource(OmegaResourceMixin, AsyncResponseMixin, Resource):
    ...
    def get(self, resource_id, action):
        ...
        return self.create_response_from_resource(
            "_generic_service_resource", action, "service", resource_id
        )
...
class JobResource(OmegaResourceMixin, AsyncResponseMixin, Resource):
    ...
    def get(self, job_id):
        ...
        return self.create_response_from_resource(
            "_generic_job_resource", action, "job", job_id
        )
    ...
    def post(self, job_id):
        return self.create_response_from_resource(
            "_generic_job_resource", "run", "job", job_id
        )
...
class DatasetResource(OmegaResourceMixin, Resource):
    ...
    def put(self, dataset_id):
        ...
        df = pd.DataFrame.from_dict(api.payload.get("data"), orient=orient).astype(
            dtypes
        )
