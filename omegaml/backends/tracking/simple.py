...
def dtrelative(delta, now=None):
    ...
    if isinstance(delta, str):
        try:
            past = -1 if delta.startswith('-') else 1
            delta = delta.replace('-', '').replace('+', '')  # Remove the sign
            num = int(delta[:-1])  # The numeric part
            units = UNIT_MAP.get(delta[-1].replace('M', 'n').lower())  # The last character
            dtdelta = timedelta(seconds=num * units)
        except:
