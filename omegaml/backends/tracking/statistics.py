...
class ExperimentStatistics:
        ...
        def stats(time_data):
            ...
            if percentiles:
                ...
                duration = (
                    ...
                    .reset_index()
                    .merge(
                        (duration.reset_index(name='latency')),
                        on=groupby,
                    )
                )
            return duration
    ...
    def throughput(self, time_key=None, tp_unit=None, groupby=None, time_events=None, percentiles=None, **kwargs):
        ...
        def stats(time_data):
            throughput = (
                ...
                .groupby(groupby)
                .apply(
                    lambda v: tp_unit / max((v['dt'].max() - v['dt'].min()).total_seconds(), 1),
                )
    ...
    def group_latency(self, time_events=None, percentiles=None, time_slots=None, **kwargs):
        ...
        latency = (
            ...
            .groupby(bins)
            .apply(
                lambda v: max((v['dt'].max() - v['dt'].min()).total_seconds(), 1),
            )
            .describe(percentiles=percentiles)
            .to_frame()
            .T
    ...
    def utilization(self, tp_unit=None, time_events=None, percentiles=None, **kwargs):
        ...
        throughput_eff = (
            ...
            .apply(
                lambda v: (len(v) // 2) / max((v['dt'].max() - v['dt'].min()).total_seconds() * tp_unit, 1),
            )
            .describe(percentiles=percentiles)
            .to_frame()
            .T
