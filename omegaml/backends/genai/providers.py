...
class MultimodalProvider(Provider):
    ...
    def speech(
        ...
        *,
        ...
        response_format: Literal["mp3", "wav", "opus"] = "wav",
        speed: float = 1.0,
        stream: bool = False,
        ...
        if stream:
            payload.update(stream_format='sse')
    ...
    def transcribe(
        ...
        *,
        ...
        stream: bool, default False
            If True, will return a SSE stream of results
        ...
        if isinstance(audio_path, (str, Path)) and Path(audio_path).exists():
            fobj = Path(audio_path).open('rb')  # ruff: ignore[open-file-with-context-handler]
            files = {"file": (Path(audio_path).name, fobj, "application/octet-stream")}
        elif hasattr(audio_path, 'read'):
            name = getattr(audio_path, 'name', 'audio.wav')
            fobj = audio_path  # type io.BytesIO
            files = {"file": (name, audio_path, "application/octet-stream")}
        elif isinstance(audio_path, bytes):
            name = 'audio.wav'
            fobj = BytesIO(audio_path)
            files = {"file": (name, fobj, "application/octet-stream")}
        else:
        ...
        *,
        ...
        return (
            ...
            if raw
                ...
                item.get("url") if response_format == "url" else item.get('b64_json') for item in data.get("data", [])
            ]
        )
