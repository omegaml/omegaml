...
class DriftMonitorBase:
    def __init__(
        self, resource=None, store=None, query=None, tracking=None, kind=None, **kwargs
    ):
        self.store = store
        ...
        self.max_corr_columns = (
            50  # maximum number of columns for correlation calculation = 50
        )
        ...
        if seq in (None, "recent"):
            drifts = [
                self._calculate_drift(
                    seq=seq, d1=d1, d2=d2, ci=ci, raw=True, matcher=matcher, since=since
                )
            ]
        elif isinstance(seq, (list, tuple)) and len(seq) > 1:
            # [0, 1, 2, ...] => compare each snapshot to the previous
            drifts = [
                self._calculate_drift(seq=[i, j], ci=ci, raw=True, since=since)
            ...
            drifts = [
                self._calculate_drift(seq=pair, ci=ci, raw=True, since=since)
                for pair in seq
            ]
        else:
            raise ValueError(
    ...
    def _calculate_drift(
        self, seq=None, d1=None, d2=None, ci=0.95, raw=False, matcher=None, since=None
    ):
        ...
        if since == "last":
            ...
            since = (
                snapshots[-1]["info"].get("since") or snapshots[seq[0]]["info"]["dt"]
            )
    ...
    def _filter_data(self, run="all", event="snapshot", since=None):
        df = self.tracking.data(
            run=run, event=event, key=self._resource, kind=self._kind, since=since
        )
    ...
    def _do_snapshot(
        ...
        numeric_columns = list(
            set(df1.select_dtypes(include="number").columns) - set(catcols)
        )
        ...
        info = snapshot.setdefault(
            "info", self._snapshot_info(name, kind, **extra_info)
        )
        ...
        corr_columns = (
            correlate if isinstance(correlate, (list, tuple)) else numeric_columns
        )
        if correlate and corr_limits:
            ...
            for method in ["pearson", "spearman"]:
                try:
                    correlations[method] = (
                        df1[corr_columns].corr(method=method).to_dict()
                    )
                except:
            ...
            if not corr_limits:
                warnings.warn(
                    f"too many columns for correlation calculation, max columns = {self.max_corr_columns}"
                )
            for method in ["pearson", "spearman"]:
                correlations[method] = None
        # -- summary statistics
        for col in numeric_columns:
            ...
            stats[s_col] = {
                ...
                "corr": {
                    method: tryOr(lambda: correlations[method][col], None)
    ...
    def _log_snapshot(self, snapshot):
        ...
        self.tracking.log_event(
            "snapshot", self._resource, snapshot, kind=snapshot["info"]["kind"]
        )
    ...
    def _calc_drift(self, s1, s2, ci=0.95, matcher=None):
        ...
        _c = (
            lambda d, s, c: matcher.get(d, matcher).get(c, c)
            if c not in s["stats"]
            else c
        )
        ...
        for col in numeric_columns + cat_columns:
            ...
            mean_stats = {}
            mean_stats["metric"] = np.mean(
                [v["drift"] for v in col_stats.values() if v]
            )
    ...
    def events(
        ...
        RETRIEVERS = {
            ...
            "snapshot": lambda *args, **kwargs: self.refresh(**kwargs)
            or (self.df if not raw else self.data),
        }
    ...
    def _get_drift_events(
        self, run=None, since=None, column=None, statistic=None, stats=False, raw=False
    ):
    ...
    def _get_alert_events(
        self, run=None, column=None, statistic=None, since=None, raw=False, stats=False
    ):
        ...
        run = run or "*"
        data = self.tracking.data(
            run=run, event="alert", key=self._drift_alert_key, since=since
        )
        if stats:
            ...
            drifted_seqs = (
                ...
                .apply(
                    lambda v: v["value"]["info"]["seq"]
                )  # get all seqs as a list of lists
    ...
    def _dataset_as_dataframe(self, dataset, rename=None, filter=None, **query):
        ...
        def _ensure_dataframe(df):
            ...
            assert isinstance(
                df, pd.DataFrame
            ), f"dataset {dataset!r} cannot be processed as a DataFrame"
            return df
        ...
        assert len(
            df
        ), f"dataset {dataset}, using query {query}, is empty, cannot take a snapshot"
        if rename:
