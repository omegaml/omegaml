...
class DataDriftMonitor(DriftMonitorBase):
    def __init__(self, dataset=None, store=None, query=None, tracking=None, kind=None, **kwargs):
        kind = kind or 'data'
        super().__init__(resource=dataset, store=store, query=query, tracking=tracking, kind=kind, **kwargs)
    ...
    def snapshot(
        ...
        **query,
        ...
        snapshot = self._do_snapshot(
            df, columns=columns, name=name, kind=kind, prefix=prefix, catcols=catcols, correlate=correlate
        )
        if groupby:
