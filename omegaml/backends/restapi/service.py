...
class GenericServiceResource:
    ...
    def predict(self, model_id, query, payload):
        ...
        result = (
            self.prepare_result(promise.get(), model_id=model_id)
    ...
    def predict_proba(self, model_id, query, payload):
        ...
        self.om.models.validate(model_id, Y=result) if isinstance(
            result, dict
        ) else None
        return result
        ...
        def do_action(self, resource_id, query, payload):
            ...
            for method, http_methods in actions.items():
                ...
                if meta.prefix == "models/":
                    ...
                    raise ValueError(
                        f"No default action for {meta.prefix}/{resource_id}"
                    )
    ...
    def run(self, script_id, query, payload):
        promise = self.om.runtime.script(script_id).run(
            payload, __format="python", **query
        )
        result = (
            self.prepare_result(promise.get(), script_id=script_id)
            if not self.is_async
            else promise
        )
        return result
    ...
    def prepare_result(
        self, result, model_id=None, script_id=None, resource_name=None, **kwargs
    ):
