...
class ParallelStep(object):
    ...
    def __call__(self, value, **kwargs):
        values = Parallel(n_jobs=self.n_jobs)(
            delayed(pfn)(value, **kwargs) for pfn in self.steps
        )
...
class DataPipeline(object):
    ...
    def map(self, values, **kwargs):
        ...
        finalizer = (
            self.steps[-1] if len(self.steps) > 1 else lambda values, **kwargs: values
        )
        pfn = DataPipeline(
            *self.args, steps=parallel_steps, context=self.context, **self.kwargs
        )
...
class Model:
    ...
    def query(self, *args, sql=None, chunksize=None, trusted=False, **vars):
        ...
        return self.store.get(
            self.name, chunksize=chunksize, sqlvars=vars, sql=sql, trusted=trusted
        )
    ...
    def join(
        ...
        **kwargs,
        ...
        sqlvars = {
            ...
            "_join_sqltable": self.store.get_backend(self.name)._default_table(
                other.table or other.name
            ),
            "_join_cond": _join_cond,
        }
        return self.query(
            sql=join_sql,
            trusted=self.store.get_backend(self.name).sign(sqlvars),
            **sqlvars,
        )
