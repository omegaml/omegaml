...
is_test_run |= len(set(test_runners) & set(cmd_args)) and "omegaml-ce" in str(
    Path().cwd()
)
...
OMEGA_MONGO_URL = (
    os.environ.get("OMEGA_MONGO_URL")
    or os.environ.get("MONGO_URL")
    or "mongodb://admin:foobar@localhost:27017/omega"
)
...
OMEGA_BROKER = (
    os.environ.get("OMEGA_BROKER")
    or os.environ.get("RABBITMQ_URL")
    or "amqp://admin:foobar@localhost:5672//"
)
...
OMEGA_WORKER_LABEL = os.environ.get("OMEGA_WORKER_LABEL") or os.environ.get(
    "CELERY_Q", "default"
)
...
def update_from_obj(obj, vars=globals(), attrs=None):
    ...
    set_default = (
        lambda o, k, d: setattr(o, k, getattr(o, k, d) or d)
        if as_attrs(o)
        else o.__setitem__(k, o.get(k) or d)
    )
    # update any
...
def load_user_extensions(vars=globals()):
    ...
    for k, v in extensions.items():
        ...
        try:
            ...
            k_type = type(v)
            msg = (
                "user extensions error: cannot apply {k} to {omvar}, "
                "expected type {omvar_type} got {k_type}"
            ).format(**locals())
...
def load_framework_support(vars=globals()):
    ...
    if tensorflow_available():
        ...
        os.environ["TF_CPP_MIN_LOG_LEVEL"] = (
            os.environ.get("TF_CPP_MIN_LOG_LEVEL") or "3"
        )
