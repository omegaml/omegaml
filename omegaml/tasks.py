...
def omega_predict(self, modelname, Xname, rName=None, pure_python=True, **kwargs):
    result = self.get_delegate(modelname).perform(
        "predict", *self.delegate_args, **self.delegate_kwargs
    )
...
def omega_reduce(self, results, modelName=None, rName=None, pure_python=True, **kwargs):
    result = self.get_delegate(modelName).perform(
        "reduce", modelName, results, **self.delegate_kwargs
    )
    return sanitized(result)
...
def omega_predict_proba(self, modelname, Xname, rName=None, pure_python=True, **kwargs):
    result = self.get_delegate(modelname).perform(
        "predict_proba", *self.delegate_args, **self.delegate_kwargs
    )
    return sanitized(result)
...
def omega_fit(self, modelname, Xname, Yname=None, pure_python=True, **kwargs):
    result = self.get_delegate(modelname).perform(
        "fit", *self.delegate_args, **self.delegate_kwargs
    )
    return sanitized(result)
...
def omega_partial_fit(self, modelname, Xname, Yname=None, pure_python=True, **kwargs):
    result = self.get_delegate(modelname).perform(
        "partial_fit", *self.delegate_args, **self.delegate_kwargs
    )
    return sanitized(result)
...
def omega_score(
    ...
    result = self.get_delegate(modelname).perform(
        "score", *self.delegate_args, **self.delegate_kwargs
    )
    return sanitized(result)


@shared_task(base=OmegamlTask, bind=True)
def omega_fit_transform(
    self, modelname, Xname, Yname=None, rName=None, pure_python=True, **kwargs
):
    result = self.get_delegate(modelname).perform(
        "fit_transform", *self.delegate_args, **self.delegate_kwargs
    )
    return sanitized(result)
...
def omega_transform(self, modelname, Xname, rName=None, **kwargs):
    result = self.get_delegate(modelname).perform(
        "transform", *self.delegate_args, **self.delegate_kwargs
    )
    return sanitized(result)
...
def omega_decision_function(self, modelname, Xname, rName=None, **kwargs):
    result = self.get_delegate(modelname).perform(
        "decision_function", *self.delegate_args, **self.delegate_kwargs
    )
    return sanitized(result)
...
def omega_gridsearch(self, modelname, Xname, Yname=None, parameters=None, **kwargs):
    result = self.get_delegate(modelname).perform(
        "gridsearch", *self.delegate_args, **self.delegate_kwargs
    )
    return sanitized(result)
