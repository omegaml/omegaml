...
class Operation:
    ...
    def add_parameter(
        self, location, name, schema=None, description=None, required=False
    ):
...
class SpecFromResourceHelperBase:
    ...
    def match_data_orient(self, datatype, orient):
        ...
        return (
            fields.List(fields.Nested(datatype))
            if orient == "records"
            else fields.Nested(datatype)
        )
    ...
    def render(self):
        ...
        if InputSchema is None:
            ...
            warnings.warn(
                f"{self.name} does not specify a signature for input (X), assuming an empty object is valid."
            )
        ...
        OutputSchema = (
            datatypes.get("Y") or datatypes.get("result") or DefaultOutputSchema
        )
    ...
    def render_operations(self, input_schema, responses):  # add operations
        ...
        responses = (
            {200: responses} if isinstance(responses, Schema) else dict(responses)
        )
        operations = []
        ...
        for action, http_methods in actions.items():
            for http_method in http_methods:
                ...
                opspec = Operation(
                    self.spec, http_method, operation_id=operation_id
            ...
            self.spec.path(
                self.path_template.format(**locals()),
    ...
    def datatypes(self, orient):
        ...
        if signature:
            ...
            datatypes = {
                ...
                if (
                    k in ("X", "Y", "result") and (spec or {}).get("schema")
                )  # skip empty schemas
            }
        ...
        if not datatypes:
            raise ValueError(
                f"Missing signature specification for object {store.prefix}/{name}"
            )
        return datatypes
...
class SpecFromModelHelper(SpecFromResourceHelperBase):
    ...
    def datatypes(self, orient):
        ...
        datatypes_["X"] = Schema.from_dict(
            {
                "data": self.match_data_orient(
                    datatypes_.get("X", Schema.from_dict({}, name="EmptyX")), orient
                ),
