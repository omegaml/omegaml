...
class OmegaTrackingProxy(RuntimeProxyBase):
    ...
    def __init__(
        self, experiment=None, provider=None, runtime=None, implied_run=True, recreate=False, **tracker_kwargs
    ):
    ...
    def create_experiment(self, name, *args, provider=None, **kwargs):
        ...
        meta = om.models.put(tracker, name, kind=ExperimentBackend.KIND, noversion=True)
    ...
    def experiment(self):
        ...
        if self._tracker is None:
            ...
            exp = om.models.get(str(fqdn), data_store=om.datasets) or self.create_experiment(
                self._experiment, provider=self.provider, **self._tracker_kwargs
            )
            self._tracker = exp
