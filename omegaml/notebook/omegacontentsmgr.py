...
class OmegaStoreContentsManager(ContentsManager):
    ...
    def get(self, path, content=True, type=None, format=None):
        ...
        if type == "notebook" or (type is None and path.endswith(".ipynb")):
            ...
            raise web.HTTPError(
                404, "Type {} at {} is not supported".format(type, path)
            )
        return model
    ...
    def dir_exists(self, path=""):
        ...
        if path == "":
            return True
        pattern = r"^{path}.*/({placeholder}|.+)".format(
            path=path, placeholder=self._dir_placeholder
        )
    ...
    def _dir_model(self, path, content=True):
        ...
        entries = self.omega.jobs.list(
            regexp=pattern, raw=True, hidden=True, include_temp=True
        )
