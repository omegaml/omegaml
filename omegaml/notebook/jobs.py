...
from omegaml.notebook.uniquejob import resolve_unique_key, JobUniqueKey
...
class NotebookMixin:
    ...
    def run(self, name, event=None, timeout=None):
        ...
        * ``results`` (str): name of results in case of OK, else blank
        ...
        Args:
            ...
            unique (str|strftime pattern, optional): ensures single execution during the 

               specified time period. Supported patterns include: 'today', 'tomorrow', 

               'yesterday', weekday names ('Monday' through 'Sunday'), month names 

               ('January' through 'December'), or strftime format strings (e.g., 

               '%Y-%m' for monthly, '%Y-W%V' for weekly).
    ...
    def run_notebook(self, name, event=None, timeout=None, unique=None):
        ...
        Args:
            ...
            unique (str|strftime pattern, optional): ensures single execution during the 
               ...
               'yesterday', weekday names ('Monday', 'Tuesday', etc.), month names 

               ('January', 'February', etc.), or strftime format strings.
        ...
        .. versionadded:: NEXT
            ...
            Added ``unique`` parameter for idempotent execution (#600)
        ...
        .. versionadded:: NEXT
            ...
            Jobs can now be scheduled with unique (= idempotent) execution support (#600).

            Use ``unique`` to specify a time period during which the job will only 

            execute once (e.g., ``unique='today'`` for once per day, or 

            ``unique='%Y-%m'`` for once per month).
        ...
        .. versionadded:: NEXT
            ...
            The ``unique`` parameter ensures idempotent execution (#600).

        .. versionadded:: NEXT

            Added ``unique`` parameter support for idempotent execution (#600).
        ...
        # Check & register unique execution constraint (#600)

        effective_unique = unique or meta_job.attributes.get('unique', {}).get('value')

        if effective_unique:

            from omegaml.notebook.uniquejob import (

                resolve_unique_key, JobUniqueKey, check_and_register_execution

            )

            try:

                # Resolve the unique key value for this period

                resolved = resolve_unique_key(name, effective_unique)

                has_run, run_meta = check_and_register_execution(name, resolved)

                if has_run:

                    raise JobAlreadyExecuted(

                        f"Job '{name}' already executed uniquely during this period ({effective_unique})."

                    )

            except JobAlreadyExecuted:

                raise

            except Exception as exc:  

                self.logger.warning(f"Error checking unique execution for '{{name}}': {{exc}}")
    ...
    def schedule(self, nb_file, run_at=None, last_run=None, unique=None):
        ...
        Args:
            ...
            unique (str|strftime pattern, optional): ensures single execution during 

               the specified time period. Supported patterns: 'today', 'tomorrow', 
        ...
        See Also:
            ...
            * OmegaJobs.get_schedule() -- for retrieving schedule info

            * unique (str|strftime pattern) -- allows specifying single 

              execution during a time window (e.g., 'today', '%Y-%m')

        .. versionadded:: NEXT

            Added ``unique`` parameter support (#600).

        .. versionadded:: NEXT

            Added ``unique`` parameter for idempotent/exclusive execution of scheduled jobs (#600).        """
        ...
        # Handle unique execution key specification (#600)
        ...
        if unique:
            ...
            try:
                ...
                # Resolve the unique key value for scheduling

                resolved = resolve_unique_key(nb_file, unique)

                meta_unique = JobUniqueKey(nb_file, resolved)

                attrs['unique'] = {

                    'value': unique,

                    'resolved_key': meta_unique.key

                }

            except Exception as exc:

                self.logger.warning(f"Could not resolve unique key '{{unique}}' for job '{{nb_file}}': {{exc}}")
