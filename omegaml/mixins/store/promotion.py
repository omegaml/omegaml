    ...
    * virtualobj.dill - uses the `export` promotion, effectively copying all
    ...
    def promote(
        ...
        **kwargs,
        ...
        assert isinstance(
            other, OmegaStore
        ), f"specify promote(..., other, ...) as om.models, not {type(other)}"
    ...
    def promotion_methods(
        ...
        **kwargs,
        ...
        PROMOTION_METHODS = {
            "getput": partial(
                self._get_put_promotion, name, other, asname=asname, drop=drop
            ),
            "data": partial(
                self._data_promotion,
                name,
                other,
                asname=asname,
                get=get,
        ...
        PROMOTION_METHODS["default"] = lambda *args, **kwargs: default_method(
            *args, **kwargs
        )
        return PROMOTION_METHODS

    def _get_put_promotion(
        self, name, other, asname=None, drop=None, get=None, put=None, **kwargs
    ):
    ...
    def _metadata_promotion(self, name, other, asname=None, drop=False, **kwargs):
        ...
        asname = asname or meta.name
        other.drop(asname, force=True) if drop else None
        other_meta = other.make_metadata(
            asname, meta.kind, bucket=other.bucket, prefix=other.prefix, **kwargs
        )
