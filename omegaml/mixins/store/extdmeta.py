...
class SignatureMixin:
    ...
    def link_datatype(
        ...
        **kwargs,
        ...
        Rmany, result = (
            (True, many_type(result)) if is_many(result) else (False, result)
        )
        iter_errors = (
            errors.items()
        ...
        ExceptionSchema = Schema.from_dict(
            {"message": fields.String()}, name=f"{name}Exception"
        )
        meta.attributes["signature"] = {
            ...
            "errors": {
                str(status): {
                    ...
                    "schema": self._schema_from_datatype(
                        E if is_schema(E) else ExceptionSchema
                    ),
                    "many": many,
    ...
    def link_swagger(self, specs, operations=None):
        ...
        assert (
            specs.get("swagger") == "2.0"
        ), f"Only swagger version 2.0 is currently supported."
        paths = specs["paths"]
        ...
        for path, method, opspec in pathspecs():
            ...
            for prmspec in params:
                ...
                schemaRef = (
                    schemaSpec.get("$ref")
                    if not Xis_many
                    else schemaSpec["items"].get("$ref")
                )
                Xschema = (
                    schemaSpec
                    if schemaRef is None
                    else definitions.get(schemaRef.split("/")[-1])
                )
                break
            else:
            ...
            for code, respspec in resps.items():
                if code == "200":
                    ...
                    Edtype = self._datatype_from_schema(
                        Eschema, name=f"{name}_{code}", many=Eis_many
                    )
            ...
            if Yschema is None:
                ...
                Yis_many = None
            # finally link
            Xdtype = self._datatype_from_schema(
                Xschema, name=f"{name}Input_{action}", many=Xis_many
            )
            Ydtype = self._datatype_from_schema(
                Yschema, name=f"{name}Output_{action}", many=Yis_many
            )
            meta = self.link_datatype(
                name, X=Xdtype, Y=Ydtype, actions=actions, errors=errors
            )
            metas.append(meta)
        return metas
    ...
    def validate(self, name, X=None, Y=None, result=None, error=None, **kwargs):
        ...
        def _do_validate(k, v):
            ...
            if isinstance(v, Exception) or k == "error":
                ...
                v = (
                    v.args[0] if len(v.args) > 0 else v
                )  # parse exception value instead of instance
    ...
    def _datatype_from_schema(self, schema, name=None, orient="records", many=False):
        ...
        for prop, pspec in prop.items():
            ...
            ftype = (
                TYPE_MAP.get(fptype) or TYPE_MAP.get(ptype) or TYPE_MAP.get("default")
            )
...
class ModelSignatureMixin(SignatureMixin):
    ...
    def link_dataset(
        ...
        **kwargs,
        ...
        metaX = (
            (Xmeta or data_store.metadata(Xname)).to_mongo()
            if Xname and data_store
            else None
        )
        metaY = (
            (Ymeta or data_store.metadata(Yname)).to_mongo()
            if Yname and data_store
            else None
        )
        model_attrs = {
            "Xname": Xname,
    ...
    def _post_fit_link_dataset_(
        self, result, modelname, Xname, meta=None, data_store=None, **kwargs
    ):
        meta = meta or self.metadata(modelname, **kwargs)
        self.link_dataset(
            modelname, Xname=Xname, meta=meta, data_store=data_store, **kwargs
        )
        return result
