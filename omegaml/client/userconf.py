...
def _get_omega_from_apikey(userid, apikey, api_url=None, requested_userid=None, qualifier=None, view=False):
    """
    ...
    if api_url.startswith('http') or any('test' in v for v in sys.argv):
        configs = auth_env.get_userconfig_from_api(
            requested_userid=requested_userid, api_url=api_url, view=view, defaults=defaults
        )
...
def _save_userconfig_from_apikey(
    configfile, userid, apikey, api_url=None, requested_userid=None, view=False, keys=None, qualifier=None
):
    from omegaml import settings
