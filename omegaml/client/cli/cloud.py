...
class CloudCommandBase(CommandBase):
    ...
    def login(self):
        ...
        auth_env.save_userconfig_from_apikey(
            configfile, userid, apikey, qualifier=qualifier, api_url=api_url
        )
    ...
    def _request_service_api(self, om, method, service=None, data=None, uri=None):
        ...
        # SEC: CWE-918
        # -- status: resolved
        # -- explain: url is validated according to OWASP recommendation
        sec_validate_url(url)
    ...
    def database(self):
        ...
        dump_cmd = f"mongodump --uri {shlex.quote(mongo_url)} --gzip --archive={path} {options}"
        restore_cmd = f"mongorestore --uri {shlex.quote(mongo_url)} --gzip --archive={path} --drop {options}"
        cmd = dump_cmd if backup else restore_cmd
        if restore:
            answer = self.ask(
                f"Are you sure to restore to {mongo_url} from {path}?", options="Yes,No"
            )
