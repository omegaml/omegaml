...
class OmegaCloud(CoreOmega):
    ...
    * :code:`scripts` - access to lambda modules stored and executed in the cluster
    ...
    def __repr__(self):
        return "OmegaCloud(bucket={}, auth={})".format(
            self.bucket or "default", repr(self.auth)
        )
...
def setup(
    ...
    om = auth_env.get_omega_from_apikey(
        userid=userid, apikey=apikey, api_url=api_url, qualifier=qualifier, view=view
    )
    ...
    if isinstance(config_file, str) and os.path.exists(config_file):
        with open(config_file, "r") as fin:
            ...
            if isinstance(userconfig, dict) and "OMEGA_USERID" in userconfig:
                ...
                raise SystemError(
                    "No cloud cloud userid/apikey found in config file {}.".format(
                        config_file
                    )
                )
            return omega
