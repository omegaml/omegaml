...
find $src_dir/omegaml -type f -name '*.rst' -o -name '*.py' | xargs echo sed -i "s/\bNEXT\b?/${VERSION}/g"
find $src_dir/docs/source -type f -name '*.rst' -o -name '*.py' | xargs echo sed -i "s/\bNEXT\b?/${VERSION}/g"
echo "INFO Done."
